...
class FileRenderCache:
    ...
    def _path(self, project_id: ProjectId, cache_key: str) -> Path:
        return self._root / project_id.value / "cache" / f"{cache_key}.png"

    def get(self, project_id: ProjectId, cache_key: str) -> RenderedRegion | None:
        path = self._path(project_id, cache_key)
        try:
    ...
    def put(self, project_id: ProjectId, cache_key: str, rendered: RenderedRegion) -> None:
        ...
        target = directory / f"{cache_key}.png"
