...
from infrastructure.image_processor.operations.crop import CropOperation, CROP_SPEC, validate_crop
    ...
    def test_validate_rejects_negative_position(self) -> None:
        for key in ("x", "y"):
            ...
            with self.subTest(key=key), self.assertRaises(ValueError):
                CropOperation().validate(options)
...
class CropOperationTests(unittest.TestCase):
    def test_render_sub_region_has_expected_dimensions(self) -> None:
        region = RenderedRegion(region_bytes(), 100, 100)
        result = CropOperation().render(region, {"x": 0.25, "y": 0.25, "width": 0.5, "height": 0.5})
    ...
    def test_render_full_region_preserves_dimensions(self) -> None:
        region = RenderedRegion(region_bytes(), 100, 100)
        result = CropOperation().render(region, {"x": 0, "y": 0, "width": 1, "height": 1})
