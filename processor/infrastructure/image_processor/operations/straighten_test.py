...
class StraightenOperationTests(unittest.TestCase):
    def test_validate_canonicalizes_angle_to_tenths(self) -> None:
        self.assertEqual({"angle": 1.5}, StraightenOperation().validate({"angle": 1.52}))
        self.assertEqual({"angle": -3.2}, StraightenOperation().validate({"angle": -3.24}))

    def test_validate_normalizes_zero(self) -> None:
        self.assertEqual({"angle": 0.0}, StraightenOperation().validate({"angle": 0}))
        self.assertEqual({"angle": 0.0}, StraightenOperation().validate({"angle": 0.04}))

    def test_validate_rejects_abs_over_45(self) -> None:
        with self.assertRaises(ValueError):
            StraightenOperation().validate({"angle": 45.1})
        with self.assertRaises(ValueError):
            StraightenOperation().validate({"angle": -45.1})

    def test_validate_rejects_non_real(self) -> None:
        with self.assertRaises(ValueError):
            StraightenOperation().validate({"angle": "x"})

    def test_validate_rejects_bool(self) -> None:
        with self.assertRaises(ValueError):
            StraightenOperation().validate({"angle": True})

    def test_render_zero_angle_returns_unchanged(self) -> None:
        region = RenderedRegion(region_bytes(4, 3), 4, 3)
        result = StraightenOperation().render(region, {"angle": 0.0})
    ...
    def test_render_nonzero_angle_expands_canvas(self) -> None:
        ...
        result = StraightenOperation().render(region, {"angle": 10.0})
        rad = math.radians(10)
