...
class RenderView:
    ...
    def render(self, raw_project_id: str, view_id: int) -> bytes:
        ...
        return self._render_view(image, size, view, project_id_or_not_found(raw_project_id))

    def preview(self, raw_project_id: str, view_id: int, pipeline: Pipeline) -> bytes:
        view, image, size = self._load(raw_project_id, view_id)
        return self._render_view(image, size, replace(view, pipeline=pipeline), project_id_or_not_found(raw_project_id))
    ...
    def _render_view(self, image, size, view, project_id) -> bytes:
        try:
            width, height = size
