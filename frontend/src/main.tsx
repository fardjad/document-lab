...
type Slice = { id: number; name: string; rectangle: Rectangle; rotation: number; straighten: number };
...
function rotatedBounds(rectangle: Rectangle, rotation: number) {
  ...
  return { width: Math.abs(rectangle.width * Math.cos(radians)) + Math.abs(rectangle.height * Math.sin(radians)), height: Math.abs(rectangle.width * Math.sin(radians)) + Math.abs(rectangle.height * Math.cos(radians)) };
}
...
function Viewer({ project, slice, editor, saving, straightenDraft, rootSelection, onConfirm, onCancel, onControlsReady }: { project?: Project; slice?: Slice; editor: EditorMode | null; saving: boolean; straightenDraft: number | null; rootSelection: number; onConfirm: (rectangle: Rectangle) => void; onCancel: () => void; onControlsReady: (controls: ViewerControls | null) => void }) {
  const zoom = useRef(1);
  ...
  const rotation = displayRect && slice && !editor ? ((slice.rotation + (straightenDraft ?? slice.straighten)) % 360 + 360) % 360 : 0;
  ...
  }, [project?.id, slice?.id, slice?.rotation, slice?.straighten, straightenDraft, editor?.kind]);
  useEffect(() => {
  ...
  return <section className={`viewer${editor ? " drawing" : ""}`} ref={viewport} onWheel={wheel} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onAuxClick={(event) => { if (event.button === 1) event.preventDefault(); }}>
    ...
    {straightenDraft !== null && slice && !editor && <div className="straighten-cross" aria-hidden="true"><i /><b /></div>}
...
function ProjectTree({ projects, selectedProjectId, selectedSliceId, expandedItems, straightenActive, onProject, onExpanded, onSlice, onEdit, onDelete, onRename }: { projects: Project[]; selectedProjectId: string; selectedSliceId: number | null; expandedItems: string[]; straightenActive: boolean; onProject: (id: string) => void; onExpanded: (items: string[]) => void; onSlice: (projectId: string, id: number) => void; onEdit: (slice: Slice) => void; onDelete: (slice: Slice) => void; onRename: (slice: Slice, name: string) => void }) {
  ...
  return <ThemeProvider theme={treeTheme}><SimpleTreeView className="project-tree" aria-label="Projects" expandedItems={expandedItems} selectedItems={selectedSliceId === null ? (selectedProjectId ? treeProjectId(selectedProjectId) : null) : treeSliceId(selectedProjectId, selectedSliceId)} expansionTrigger="iconContainer" onExpandedItemsChange={(_, items) => onExpanded(items)} onSelectedItemsChange={(_, item) => { if (!item) return; if (item.startsWith("project:")) onProject(item.slice(8)); else { const [, projectId, id] = item.split(":"); onSlice(projectId, Number(id)); } }}>
    {projects.map((project) => <TreeItem key={project.id} itemId={treeProjectId(project.id)} label={<span className="tree-project-label"><span className="project-dot" />{project.name}</span>}>
      ...
      {project.slices.map((slice) => <TreeItem key={slice.id} itemId={treeSliceId(project.id, slice.id)} label={<span className="tree-slice-label"><span className="slice-marker" />{renaming === slice.id ? <input autoFocus value={name} onInput={(event) => setName((event.target as HTMLInputElement).value)} onKeyDown={(event) => { if (event.key === "Enter") { onRename(slice, name); setRenaming(null); } if (event.key === "Escape") setRenaming(null); }} onBlur={() => setRenaming(null)} onClick={stopSelection} onMouseDown={stopSelection} aria-label="Slice name" /> : <span>{slice.name}</span>}{!straightenActive && <span className="slice-menu" onClick={stopSelection} onMouseDown={stopSelection}><button onClick={() => { setName(slice.name); setRenaming(slice.id); }} aria-label={`Rename ${slice.name}`}>↗</button><button onClick={() => onEdit(slice)} aria-label={`Edit ${slice.name}`}>□</button><button onClick={() => onDelete(slice)} aria-label={`Delete ${slice.name}`}>×</button></span>}</span>} />)}
    </TreeItem>)}
  </SimpleTreeView></ThemeProvider>;
}

function App() {
  const [projects, setProjects] = useState<Project[]>([]), [selectedProjectId, setSelectedProjectId] = useState(""), [selectedSliceId, setSelectedSliceId] = useState<number | null>(null), [expandedItems, setExpandedItems] = useState<string[]>([]), [loading, setLoading] = useState(true), [error, setError] = useState(""), [editor, setEditor] = useState<EditorMode | null>(null), [straightenDraft, setStraightenDraft] = useState<number | null>(null), [straightenText, setStraightenText] = useState(""), [saving, setSaving] = useState(false), [rootSelection, setRootSelection] = useState(0), [viewerControls, setViewerControls] = useState<ViewerControls | null>(null);
  ...
  useEffect(() => { if (!selectedProjectId) return; setSelectedSliceId(null); setEditor(null); setStraightenDraft(null); setStraightenText(""); setProjects((items) => items.map((item) => item.id === selectedProjectId ? { ...item, slicesLoading: true, slicesError: "" } : item)); request<Slice[]>(`${API}/projects/${encodeURIComponent(selectedProjectId)}/slices`).then((slices) => setProjects((items) => items.map((item) => item.id === selectedProjectId ? { ...item, slices, slicesLoading: false } : item))).catch((reason) => setProjects((items) => items.map((item) => item.id === selectedProjectId ? { ...item, slicesLoading: false, slicesError: reason.message } : item))); }, [selectedProjectId]);
  const selectProject = (id: string) => { setSelectedProjectId(id); setSelectedSliceId(null); setEditor(null); setStraightenDraft(null); setStraightenText(""); setRootSelection((value) => value + 1); };
  const updateSlice = async (sliceId: number, body: { name: string; rectangle: Rectangle; rotation: number; straighten: number }) => { const updated = await request<Slice>(`${API}/projects/${encodeURIComponent(selectedProjectId)}/slices/${sliceId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }); setProjects((items) => items.map((item) => item.id === selectedProjectId ? { ...item, slices: item.slices.map((itemSlice) => itemSlice.id === sliceId ? updated : itemSlice) } : item)); };
  const confirmSlice = async (rectangle: Rectangle) => { if (!project) return; setSaving(true); try { if (editor?.kind === "edit") { await updateSlice(editor.slice.id, { name: editor.slice.name, rectangle, rotation: editor.slice.rotation, straighten: editor.slice.straighten }); } else { const created = await request<Slice>(`${API}/projects/${encodeURIComponent(project.id)}/slices`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ rectangle }) }); setProjects((items) => items.map((item) => item.id === project.id ? { ...item, slices: [...item.slices, created] } : item)); setSelectedSliceId(created.id); } setEditor(null); } catch (reason) { setError(reason.message); } finally { setSaving(false); } };
  const renameSlice = async (item: Slice, name: string) => { const nextName = name.trim(); if (!nextName || nextName === item.name) return; try { await updateSlice(item.id, { name: nextName, rectangle: item.rectangle, rotation: item.rotation, straighten: item.straighten }); } catch (reason) { setError(reason.message); } };
  const rotateSlice = async (item: Slice, delta: number) => { setSaving(true); try { await updateSlice(item.id, { name: item.name, rectangle: item.rectangle, rotation: (item.rotation + delta + 360) % 360, straighten: item.straighten }); } catch (reason) { setError(reason.message); } finally { setSaving(false); } };
  ...
  const confirmStraighten = async () => { const parsed = parseStraighten(straightenText); if (!slice || straightenDraft === null || parsed === null) return; setSaving(true); try { await updateSlice(slice.id, { name: slice.name, rectangle: slice.rectangle, rotation: slice.rotation, straighten: parsed }); setStraightenDraft(null); setStraightenText(""); } catch (reason) { setError(reason.message); } finally { setSaving(false); } };
  ...
  return <main className="app-shell"><aside className="toolbar"><div className="brand"><span className="brand-mark">◩</span><span>Cropper</span></div><div className="toolbar-section"><span className="eyebrow">Projects</span>{loading && <p className="muted">Loading…</p>}{error && <p className="error">{error}</p>}{!loading && !error && projects.length === 0 && <p className="muted">No projects found.</p>}<ProjectTree projects={projects} selectedProjectId={selectedProjectId} selectedSliceId={selectedSliceId} expandedItems={expandedItems} straightenActive={straightenDraft !== null} onProject={selectProject} onExpanded={setExpandedItems} onSlice={(projectId, id) => { setEditor(null); setStraightenDraft(null); setStraightenText(""); setSelectedProjectId(projectId); setSelectedSliceId(id); }} onEdit={(item) => { setStraightenDraft(null); setStraightenText(""); setSelectedSliceId(item.id); setEditor({ kind: "edit", slice: item }); }} onDelete={deleteSlice} onRename={renameSlice} /></div><div className="toolbar-foot">Drag edge to resize<br /><span>Wheel zoom · drag pan</span></div></aside><div className="workspace"><header className={straightenDraft !== null ? "straighten-active" : ""}><div><span className="eyebrow">Workspace</span><h1>{project?.name ?? "Document viewer"}</h1></div><div className="header-tools"><button className="slice-tool" onClick={() => { setStraightenDraft(null); setStraightenText(""); setEditor({ kind: "create" }); }} disabled={!project} aria-pressed={Boolean(editor)}>＋ Slice</button>{slice && !editor && straightenDraft === null && <div className="rotation-tools"><button onClick={() => rotateSlice(slice, -90)} disabled={saving} aria-label="Rotate slice left 90 degrees" title="Rotate slice left 90 degrees">↶ 90°</button><button onClick={() => rotateSlice(slice, 90)} disabled={saving} aria-label="Rotate slice right 90 degrees" title="Rotate slice right 90 degrees">↷ 90°</button></div>}{slice && !editor && straightenDraft === null && <button className="straighten-tool" onClick={() => { setStraightenDraft(slice.straighten); setStraightenText(formatStraighten(slice.straighten)); }} disabled={saving}>Straighten</button>}{slice && !editor && straightenDraft !== null && <div className="straighten-tools"><button className="straighten-step" onClick={() => adjustStraighten(-0.1)} disabled={saving} aria-label="Decrease straighten angle by 0.1 degrees">−</button><label className="angle-control"><input className="straighten-number" value={straightenText} onInput={(event) => { const value = (event.target as HTMLInputElement).value; setStraightenText(value); const parsed = parseStraighten(value); if (parsed !== null) setStraightenDraft(parsed); }} onBlur={commitStraightenText} onKeyDown={(event) => { if (event.key === "Enter") commitStraightenText(); }} aria-label="Straighten angle" aria-valuetext={`${straightenText || "invalid"} degrees`} /><span>°</span></label><button className="straighten-step" onClick={() => adjustStraighten(0.1)} disabled={saving} aria-label="Increase straighten angle by 0.1 degrees">+</button><input className="straighten-slider" type="range" min="-45" max="45" step="0.1" value={straightenDraft ?? 0} onChange={(event) => { const value = Number(event.target.value); setStraightenDraft(value); setStraightenText(formatStraighten(value)); }} aria-label="Straighten angle slider" /><button className="straighten-action" onClick={() => { setStraightenDraft(null); setStraightenText(""); }} disabled={saving}>Cancel</button><button className="straighten-action confirm" onClick={confirmStraighten} disabled={saving || parseStraighten(straightenText) === null}>Confirm</button></div>}<div className="view-presets"><button onClick={() => viewerControls?.fit()} disabled={!viewerControls}>Fit</button><button onClick={() => viewerControls?.fitWidth()} disabled={!viewerControls}>Fit Width</button><button onClick={() => viewerControls?.actualSize()} disabled={!viewerControls}>Actual Size</button></div><div className="zoom-tools"><button onClick={() => viewerControls?.zoomOut()} disabled={!viewerControls} aria-label="Zoom out">−</button><button onClick={() => viewerControls?.zoomIn()} disabled={!viewerControls} aria-label="Zoom in">+</button></div><span className="status"><i />{project ? "Ready" : "Waiting"}</span></div></header><Viewer project={project} slice={slice} editor={editor} saving={saving} straightenDraft={straightenDraft} rootSelection={rootSelection} onConfirm={confirmSlice} onCancel={() => setEditor(null)} onControlsReady={setViewerControls} /></div></main>;
}
