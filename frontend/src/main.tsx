...
type Region = { id: number; name: string; rectangle: Rectangle; pipeline: Pipeline };
...
function Viewer({ project, region, editor, saving, straightenDraft, trimDraft, renderedUrl, previewUrl, rootSelection, onTrimValidity, onConfirm, onCancel, onControlsReady, children }: { project?: Project; region?: Region; editor: EditorMode | null; saving: boolean; straightenDraft: number | null; trimDraft: Trim | null; renderedUrl?: string; previewUrl?: string; rootSelection: number; onTrimValidity: (valid: boolean) => void; onConfirm: (rectangle: Rectangle) => void; onCancel: () => void; onControlsReady: (controls: ViewerControls | null) => void; children?: ReactNode }) {
  const zoom = useRef(1);
  ...
  const displayRect = editor || renderedView ? undefined : region ? imageRectangle(region.rectangle) : undefined;
  const rotation = displayRect && region && !editor ? ((getRotateDegrees(region.pipeline) + (straightenDraft ?? getStraightenAngle(region.pipeline))) % 360 + 360) % 360 : 0;
  const activeTrim = displayRect && region && !editor ? (trimDraft ?? getTrim(region.pipeline)) : emptyTrim();
  ...
  const handleImageLoad = () => { const nowRendered = Boolean(previewUrl ?? renderedUrl); if (region && !editor && !nowRendered) update(); else if (renderedMode.current && nowRendered) update(); else fit(); renderedMode.current = nowRendered; };
  useEffect(() => {
    onControlsReady({ fit, fitWidth, actualSize, zoomOut: () => changeZoom(0.89), zoomIn: () => changeZoom(1.12) });
    return () => onControlsReady(null);
  }, [project?.id, region?.id, region?.pipeline, straightenDraft, trimDraft, editor?.kind]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setZoomReadout(100);
    fit();
  }, [region?.id, region?.pipeline, renderedUrl, Boolean(previewUrl)]);
  useEffect(() => { if (straightenDraft !== null) update(); }, [straightenDraft]);
  useEffect(() => { const view = geometry()?.view; onTrimValidity(Boolean(view && view.width >= 4 && view.height >= 4)); update(); }, [trimDraft, region?.id, region?.pipeline, rotation]);
  useEffect(() => {
...
function ProjectTree({ projects, selectedProjectId, selectedRegionId, expandedItems, straightenActive, filter, onProject, onExpanded, onRegion, onEdit, onDelete, onRename }: { projects: Project[]; selectedProjectId: string; selectedRegionId: number | null; expandedItems: string[]; straightenActive: boolean; filter: string; onProject: (id: string) => void; onExpanded: (items: string[]) => void; onRegion: (projectId: string, id: number) => void; onEdit: (region: Region) => void; onDelete: (region: Region) => void; onRename: (region: Region, name: string) => void }) {
  const [renaming, setRenaming] = useState<number | null>(null);
  const [name, setName] = useState("");
  const query = filter.trim().toLocaleLowerCase();
  const visibleProjects = projects.map((project) => { const projectMatch = !query || project.name.toLocaleLowerCase().includes(query); const regions = projectMatch ? project.regions : project.regions.filter((region) => region.name.toLocaleLowerCase().includes(query)); return { project, regions }; }).filter(({ project, regions }) => !query || project.name.toLocaleLowerCase().includes(query) || regions.length > 0);
  ...
  return <ThemeProvider theme={treeTheme}><SimpleTreeView className="project-tree" aria-label="Projects" expandedItems={expandedItems} selectedItems={selectedRegionId === null ? (selectedProjectId ? treeProjectId(selectedProjectId) : null) : treeRegionId(selectedProjectId, selectedRegionId)} expansionTrigger="iconContainer" onExpandedItemsChange={(_, items) => onExpanded(items)} onSelectedItemsChange={(_, item) => { if (!item) return; if (item.startsWith("project:")) onProject(item.slice(8)); else { const [, projectId, id] = item.split(":"); onRegion(projectId, Number(id)); } }}>
    {visibleProjects.map(({ project, regions }) => <TreeItem key={project.id} itemId={treeProjectId(project.id)} label={<span className="tree-project-label"><span className="project-dot" />{project.name}</span>}>
...
function App() {
  ...
  const autoContext = `${region?.id ?? ""}:${autoMode}`;
  const autoRequest = useRef(0);
  const backgroundRequest = useRef(0);
  const renderedUrl = region && !editor && getRemoveBackground(region.pipeline) && trimDraft === null && straightenDraft === null && backgroundDraft === null ? `${API}/projects/${encodeURIComponent(selectedProjectId)}/regions/${region.id}/render?render=${renderToken}-${getRotateDegrees(region.pipeline)}-${getStraightenAngle(region.pipeline)}-${encodeURIComponent(JSON.stringify(getTrim(region.pipeline)))}` : undefined;
  ...
  useEffect(() => {
    ...
    if (!project || !region || !settings || settings === "none") {
    ...
    const timer = setTimeout(() => {
      ...
      fetch(`${API}/projects/${encodeURIComponent(project.id)}/regions/${region.id}/render`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ operations: setOp(region.pipeline, "remove_background", settings) }) }).then((response) => { if (!response.ok) throw new Error(`Preview failed (${response.status})`); return response.blob(); }).then((blob) => {
  ...
  }, [backgroundDraft, region?.id, project?.id]);
  const selectProject = (id: string) => { setSelectedProjectId(id); setSelectedRegionId(null); setEditor(null); setStraightenDraft(null); setStraightenText(""); setTrimDraft(null); setBackgroundDraft(null); setRootSelection((value) => value + 1); };
  const updateRegion = async (regionId: number, body: { name: string; rectangle: Rectangle; pipeline: Pipeline }) => { const updated = await request<Region>(`${API}/projects/${encodeURIComponent(selectedProjectId)}/regions/${regionId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: body.name, rectangle: body.rectangle, pipeline: { operations: body.pipeline } }) }); setProjects((items) => items.map((item) => item.id === selectedProjectId ? { ...item, regions: item.regions.map((itemRegion) => itemRegion.id === regionId ? updated : itemRegion) } : item)); };
  const confirmRegion = async (rectangle: Rectangle) => { if (!project) return; setSaving(true); try { if (editor?.kind === "edit") { await updateRegion(editor.region.id, { name: editor.region.name, rectangle, pipeline: editor.region.pipeline }); } else { const created = await request<Region>(`${API}/projects/${encodeURIComponent(project.id)}/regions`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ rectangle }) }); setProjects((items) => items.map((item) => item.id === project.id ? { ...item, regions: [...item.regions, created] } : item)); setSelectedRegionId(created.id); } setEditor(null); } catch (reason) { setError(reason.message); } finally { setSaving(false); } };
  const renameRegion = async (item: Region, name: string) => { const nextName = name.trim(); if (!nextName || nextName === item.name) return; try { await updateRegion(item.id, { name: nextName, rectangle: item.rectangle, pipeline: item.pipeline }); } catch (reason) { setError(reason.message); } };
  const rotateRegion = async (item: Region, delta: number) => { setSaving(true); try { await updateRegion(item.id, { name: item.name, rectangle: item.rectangle, pipeline: setOp(item.pipeline, "rotate", { degrees: (getRotateDegrees(item.pipeline) + delta + 360) % 360 }) }); } catch (reason) { setError(reason.message); } finally { setSaving(false); } };
  ...
  const confirmStraighten = async () => { const parsed = parseStraighten(straightenText); if (!region || straightenDraft === null || parsed === null) return; setSaving(true); try { await updateRegion(region.id, { name: region.name, rectangle: region.rectangle, pipeline: setOp(region.pipeline, "straighten", { angle: parsed }) }); setStraightenDraft(null); setStraightenText(""); setTrimDraft(null); } catch (reason) { setError(reason.message); } finally { setSaving(false); } };
  const beginTrim = () => { if (!region) return; const trim = getTrim(region.pipeline); setTrimDraft({ ...trim }); setTrimText({ ...trim }); };
  ...
  const confirmTrim = async () => { if (!region || !trimDraft || !trimValid || Object.values(trimText).some((value) => parsePixels(String(value)) === null)) return; setSaving(true); try { await updateRegion(region.id, { name: region.name, rectangle: region.rectangle, pipeline: setOp(region.pipeline, "trim", trimDraft) }); setTrimDraft(null); } catch (reason) { setError(reason.message); } finally { setSaving(false); } };
  const beginBackground = () => { if (!region) return; setStraightenDraft(null); setStraightenText(""); setTrimDraft(null); setBackgroundDraft(getRemoveBackground(region.pipeline) ?? defaultBackgroundRemoval()); };
