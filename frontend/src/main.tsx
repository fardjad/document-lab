...
function Viewer({ project, region, editor, saving, straightenDraft, trimDraft, rootSelection, onTrimValidity, onConfirm, onCancel, onControlsReady, children }: { project?: Project; region?: Region; editor: EditorMode | null; saving: boolean; straightenDraft: number | null; trimDraft: Trim | null; rootSelection: number; onTrimValidity: (valid: boolean) => void; onConfirm: (rectangle: Rectangle) => void; onCancel: () => void; onControlsReady: (controls: ViewerControls | null) => void; children?: ReactNode }) {
  const zoom = useRef(1);
...
function App() {
  ...
  const autoMode = editor ? "none" : straightenDraft !== null ? "straighten" : trimDraft !== null ? "trim" : "none";
