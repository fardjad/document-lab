...
function App() {
  ...
  const add = (kind: string) => { const m = opMeta(metas, kind); if (!m) return; const next = [...pipeline, { kind, options: { ...m.default_options }, enabled: true }]; if (kind === "crop") next.unshift(next.pop()!); setPipeline(next); setSelected(kind === "crop" ? 0 : next.length - 1); setActiveEditing(true); };
  ...
  const upload = async (e: ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (!file) return; const fd = new FormData(); fd.append("image", file); try { const p = await request<Project>(`${API}/projects`, { method: "POST", body: fd }); setProjects(ps => [...ps, p]); setProjectId(p.id); } catch (x) { setError((x as Error).message); } };
    ...
    <aside className={`right ${rightFolded ? "folded" : ""}`} style={{ width: rightFolded ? 30 : rightWidth }}>
      ...
      {!rightFolded && <><Button fullWidth variant="contained" startIcon={<SaveIcon />} disabled={!view || saving} onClick={save} className="save-pipeline">{saving ? "Saving…" : "Save pipeline"}</Button><Pipeline pipeline={pipeline} metas={metas} selected={selected} onSelect={setSelected} onChange={p => { setPipeline(p); setActiveEditing(true); }} onAdd={add} onRemove={remove} onMove={move} onHelper={(i, h) => { if (Object.keys(h.schema ?? {}).length) { setHelper({ index: i, value: h }); setHelperOptions({}); } else { setHelper({ index: i, value: h }); setHelperOptions({}); setTimeout(helperRun, 0); } }} /></>}
    </aside>
