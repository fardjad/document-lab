...
function App() {
  ...
  const upload = async (e: ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (!file) return; const fd = new FormData(); fd.append("image", file); try { const p = await request<Project>(`${API}/projects`, { method: "POST", body: fd }); setProjects(ps => [...ps, p]); setProjectId(p.id); } catch (x) { setError((x as Error).message); } };
    ...
    <aside className={`left ${leftFolded ? "folded" : ""}`} style={{ width: leftFolded ? 30 : leftWidth }}>
      ...
      {!leftFolded && <><Box className="section-actions"><Typography className="section-title">Projects</Typography><Button component="label" size="small" startIcon={<UploadFileIcon />}>Import<input hidden type="file" accept="image/*" onChange={upload} /></Button></Box><ProjectTree projects={projects} selectedProject={projectId} selectedView={viewId} onProject={p => { setProjectId(p); setViewId(null); }} onView={selectView} onCreate={createView} onRenameView={beginRename} onDeleteView={(p, v) => setDeleteTarget({ project: p, view: v })} onDeleteProject={p => setDeleteTarget({ project: p })} /></>}
    </aside>
