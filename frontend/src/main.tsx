...
function Preview({ project, view, pipeline, activeEditing, onImageDimensions }: { project?: Project; view?: View; pipeline: PipelineOp[]; activeEditing: boolean; onImageDimensions: (width: number, height: number) => void }) {
...
function App() {
  ...
  const add = (kind: string) => { const m = opMeta(metas, kind); if (!m) return; const next = [...pipeline, { kind, options: { ...m.default_options }, enabled: true }]; if (kind === "crop") next.unshift(next.pop()!); setPipeline(next); setSelectedOperation(kind === "crop" ? 0 : next.length - 1); setActiveEditing(true); };
  ...
  const move = (i: number, d: number) => { const j = i + d; if (j < 0 || j >= pipeline.length || (pipeline[i].kind === "crop" && j !== 0) || (pipeline[j].kind === "crop" && i !== 0)) return; const n = [...pipeline]; [n[i], n[j]] = [n[j], n[i]]; setPipeline(n); setSelectedOperation(j); setActiveEditing(true); };
  ...
  const upload = async (e: ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (!file) return; const fd = new FormData(); fd.append("image", file); try { const p = await request<Project>(`${API}/projects`, { method: "POST", body: fd }); setProjects(ps => [...ps, p]); setProjectId(p.id); } catch (x) { setError((x as Error).message); } };
    ...
    <main className="main">
      ...
      <Box className="center" style={{ gridTemplateRows: parametersFolded ? "minmax(0, 1fr) 0px 32px" : `${splitRatio * 100}% auto ${ (1 - splitRatio) * 100}%` }}>
        <Box className="center-pane"><Preview project={project} view={view} pipeline={pipeline} activeEditing={activeEditing} onImageDimensions={(width, height) => setNaturalDimensions({ width, height })} /></Box>
        ...
        <Box className={`center-pane ${parametersFolded ? "folded-pane" : ""}`}>{parametersFolded ? <FoldButton label="Expand parameters" direction="up" onClick={() => setParametersFolded(false)} /> : <><Box className="pane-corner"><FoldButton label="Collapse parameters" direction="down" onClick={() => setParametersFolded(true)} /></Box><Parameters meta={selectedMeta} op={selectedOperation === null ? undefined : pipeline[selectedOperation]} naturalWidth={naturalDimensions.width} naturalHeight={naturalDimensions.height} onChange={o => { if (selectedOperation === null) return; setPipeline(p => p.map((x, i) => i === selectedOperation ? { ...x, options: o } : x)); setActiveEditing(true); }} /></>}</Box>
      </Box>
    </main>
