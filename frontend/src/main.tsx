...
type Region = { id: number; name: string; rectangle: Rectangle; rotation: number; straighten: number; trim: Trim; background_removal?: BackgroundRemoval | null };
...
function Viewer({ project, region, editor, saving, straightenDraft, trimDraft, renderedUrl, previewUrl, rootSelection, onTrimValidity, onConfirm, onCancel, onControlsReady, children }: { project?: Project; region?: Region; editor: EditorMode | null; saving: boolean; straightenDraft: number | null; trimDraft: Trim | null; renderedUrl?: string; previewUrl?: string; rootSelection: number; onTrimValidity: (valid: boolean) => void; onConfirm: (rectangle: Rectangle) => void; onCancel: () => void; onControlsReady: (controls: ViewerControls | null) => void; children?: ReactNode }) {
  ...
  useEffect(() => {
    ...
    setZoomReadout(100);
    fit();
  }, [region?.id, region?.rotation, renderedUrl, Boolean(previewUrl)]);
...
function App() {
  ...
  const renderedUrl = region && !editor && region.background_removal && trimDraft === null && straightenDraft === null && backgroundDraft === null ? `${API}/projects/${encodeURIComponent(selectedProjectId)}/regions/${region.id}/download?render=${renderToken}-${region.rotation}-${region.straighten}-${encodeURIComponent(JSON.stringify(region.trim))}` : undefined;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      fetch(`${API}/projects/${encodeURIComponent(project.id)}/regions/${region.id}/background-removal/preview`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(settings) }).then((response) => { if (!response.ok) throw new Error(`Preview failed (${response.status})`); return response.blob(); }).then((blob) => {
  ...
  const updateRegion = async (regionId: number, body: { name: string; rectangle: Rectangle; rotation: number; straighten: number; trim: Trim; background_removal?: BackgroundRemoval | null }) => { const updated = await request<Region>(`${API}/projects/${encodeURIComponent(selectedProjectId)}/regions/${regionId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }); setProjects((items) => items.map((item) => item.id === selectedProjectId ? { ...item, regions: item.regions.map((itemRegion) => itemRegion.id === regionId ? updated : itemRegion) } : item)); };
  const confirmRegion = async (rectangle: Rectangle) => { if (!project) return; setSaving(true); try { if (editor?.kind === "edit") { await updateRegion(editor.region.id, { name: editor.region.name, rectangle, rotation: editor.region.rotation, straighten: editor.region.straighten, trim: trimValue(editor.region), background_removal: editor.region.background_removal ?? null }); } else { const created = await request<Region>(`${API}/projects/${encodeURIComponent(project.id)}/regions`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ rectangle }) }); setProjects((items) => items.map((item) => item.id === project.id ? { ...item, regions: [...item.regions, created] } : item)); setSelectedRegionId(created.id); } setEditor(null); } catch (reason) { setError(reason.message); } finally { setSaving(false); } };
  const renameRegion = async (item: Region, name: string) => { const nextName = name.trim(); if (!nextName || nextName === item.name) return; try { await updateRegion(item.id, { name: nextName, rectangle: item.rectangle, rotation: item.rotation, straighten: item.straighten, trim: trimValue(item), background_removal: item.background_removal ?? null }); } catch (reason) { setError(reason.message); } };
  const rotateRegion = async (item: Region, delta: number) => { setSaving(true); try { await updateRegion(item.id, { name: item.name, rectangle: item.rectangle, rotation: (item.rotation + delta + 360) % 360, straighten: item.straighten, trim: trimValue(item), background_removal: item.background_removal ?? null }); } catch (reason) { setError(reason.message); } finally { setSaving(false); } };
  ...
  const confirmStraighten = async () => { const parsed = parseStraighten(straightenText); if (!region || straightenDraft === null || parsed === null) return; setSaving(true); try { await updateRegion(region.id, { name: region.name, rectangle: region.rectangle, rotation: region.rotation, straighten: parsed, trim: trimValue(region), background_removal: region.background_removal ?? null }); setStraightenDraft(null); setStraightenText(""); setTrimDraft(null); } catch (reason) { setError(reason.message); } finally { setSaving(false); } };
  ...
  const confirmTrim = async () => { if (!region || !trimDraft || !trimValid || Object.values(trimText).some((value) => parsePixels(String(value)) === null)) return; setSaving(true); try { await updateRegion(region.id, { name: region.name, rectangle: region.rectangle, rotation: region.rotation, straighten: region.straighten, trim: trimDraft, background_removal: region.background_removal ?? null }); setTrimDraft(null); } catch (reason) { setError(reason.message); } finally { setSaving(false); } };
  const beginBackground = () => { if (!region) return; setStraightenDraft(null); setStraightenText(""); setTrimDraft(null); setBackgroundDraft(region.background_removal ?? defaultBackgroundRemoval()); };
  ...
  const confirmBackground = async () => { if (!region || !backgroundDraft) return; setSaving(true); try { await updateRegion(region.id, { name: region.name, rectangle: region.rectangle, rotation: region.rotation, straighten: region.straighten, trim: trimValue(region), background_removal: backgroundDraft === "none" ? null : backgroundDraft }); setBackgroundDraft(null); setRenderToken((value) => value + 1); } catch (reason) { setError(reason.message); } finally { setSaving(false); } };
  const cancelBackground = () => setBackgroundDraft(null);
  const removeBackground = async () => { if (!region || region.background_removal === undefined || region.background_removal === null) return; setSaving(true); try { await updateRegion(region.id, { name: region.name, rectangle: region.rectangle, rotation: region.rotation, straighten: region.straighten, trim: trimValue(region), background_removal: null }); setBackgroundDraft(null); setRenderToken((value) => value + 1); } catch (reason) { setError(reason.message); } finally { setSaving(false); } };
  const autoAnalyze = async (operation: "straighten" | "trim") => { if (!region) return; const requestId = ++autoRequest.current; const context = `${region.id}:${operation}`; setAutoState({ loading: true, message: "Calculating…", error: "" }); try { const response = await fetch(`${API}/projects/${encodeURIComponent(project!.id)}/regions/${region.id}/analysis`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ operation }) }); if (!response.ok) throw new Error(`Analysis failed (${response.status})`); const result = await response.json() as { suggestion: number | Trim | null; confidence?: number; reason?: string }; if (requestId !== autoRequest.current || autoContextRef.current !== context) return; if (result.suggestion === null) { setAutoState({ loading: false, message: result.reason ?? "No suggestion available", error: "" }); return; } if (operation === "straighten") { if (typeof result.suggestion !== "number" || !Number.isFinite(result.suggestion) || Math.abs(result.suggestion) > 45) throw new Error("Invalid straighten suggestion"); const value = Math.round(result.suggestion * 10) / 10; setStraightenDraft(value); setStraightenText(formatStraighten(value)); } else { const value = result.suggestion; if (!value || typeof value !== "object" || [value.top, value.right, value.bottom, value.left].some((item: unknown) => !Number.isInteger(item) || (item as number) < 0)) throw new Error("Invalid trim suggestion"); setTrimDraft(value); setTrimText(value); } const confidence = typeof result.confidence === "number" ? ` · ${Math.round(result.confidence * 100)}% confidence` : ""; setAutoState({ loading: false, message: `Suggestion ready${confidence}`, error: "" }); } catch (reason) { if (requestId === autoRequest.current && autoContextRef.current === context) setAutoState({ loading: false, message: "", error: reason instanceof Error ? reason.message : "Auto analysis failed" }); } };
  const downloadRegion = () => { if (!region || editor || straightenDraft !== null || trimDraft !== null || backgroundDraft !== null || saving) return; const link = document.createElement("a"); link.href = `${API}/projects/${encodeURIComponent(project!.id)}/regions/${region.id}/download`; link.click(); };
