...
function Preview({ project, view, pipeline, selected, onCrop }: { project?: Project; view?: View; pipeline: PipelineOp[]; selected?: Metadata; onCrop: (o: Options) => void }) {
  ...
  useEffect(() => { if (!project || !view) { setPreview(""); return; } const timer = setTimeout(() => { fetch(`${API}/projects/${encodeURIComponent(project.id)}/views/${view.id}/preview`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ pipeline }) }).then(r => r.ok ? r.blob() : Promise.reject(new Error("Preview failed"))).then(b => { const u = URL.createObjectURL(b); setPreview(old => { if (old) URL.revokeObjectURL(old); return u; }); }).catch(() => setPreview("")); }, 350); return () => clearTimeout(timer); }, [project?.id, view?.id, JSON.stringify(pipeline)]);
  const src = preview || (view && project ? `${API}/projects/${encodeURIComponent(project.id)}/views/${view.id}/render` : project?.imageUrl);
...
function App() {
  ...
  const [leftFolded, setLeftFolded] = useState(false), [rightFolded, setRightFolded] = useState(false), [previewFolded, setPreviewFolded] = useState(false), [parametersFolded, setParametersFolded] = useState(false);
  ...
  useEffect(() => { if (view) { setPipeline(clonePipeline(view.pipeline)); setSelected(0); } }, [view?.id]);
  const selectView = (p: string, v: number) => { setProjectId(p); setViewId(v); };
  const add = (kind: string) => { const m = opMeta(metas, kind); if (!m) return; const next = [...pipeline, { kind, options: { ...m.default_options } }]; if (kind === "crop") next.unshift(next.pop()!); setPipeline(next); setSelected(kind === "crop" ? 0 : next.length - 1); };
  const remove = (i: number) => setPipeline(p => p.filter((_, n) => n !== i));
  const move = (i: number, d: number) => { const j = i + d; if (j < 0 || j >= pipeline.length || (pipeline[i].kind === "crop" && j !== 0) || (pipeline[j].kind === "crop" && i !== 0)) return; const n = [...pipeline]; [n[i], n[j]] = [n[j], n[i]]; setPipeline(n); setSelected(j); };
  ...
  const upload = async (e: ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (!file) return; const fd = new FormData(); fd.append("image", file); try { const p = await request<Project>(`${API}/projects`, { method: "POST", body: fd }); setProjects(ps => [...ps, p]); setProjectId(p.id); } catch (x) { setError((x as Error).message); } };
    ...
    <main className="main">
      ...
      <Box className="center" style={{ gridTemplateRows: `${previewFolded ? 0 : splitRatio * 100}% auto ${parametersFolded ? 0 : (1 - splitRatio) * 100}%` }}>
        <Box className={`center-pane ${previewFolded ? "folded-pane" : ""}`}>{previewFolded ? <FoldButton label="Expand preview" direction="down" onClick={() => setPreviewFolded(false)} /> : <><Box className="pane-corner"><FoldButton label="Collapse preview" direction="up" onClick={() => setPreviewFolded(true)} /></Box><Preview project={project} view={view} pipeline={pipeline} selected={selectedMeta} onCrop={o => setPipeline(p => p.map((x, i) => i === selected ? { ...x, options: o } : x))} /></>}</Box>
        {!previewFolded && !parametersFolded && <ResizeHandle axis="vertical" onResize={d => setSplitRatio(r => Math.max(.2, Math.min(.8, r + d / Math.max(1, document.querySelector(".center")?.clientHeight ?? 1))))} />}
        <Box className={`center-pane ${parametersFolded ? "folded-pane" : ""}`}>{parametersFolded ? <FoldButton label="Expand parameters" direction="up" onClick={() => setParametersFolded(false)} /> : <><Box className="pane-corner"><FoldButton label="Collapse parameters" direction="down" onClick={() => setParametersFolded(true)} /></Box><Parameters meta={selectedMeta} op={pipeline[selected]} onChange={o => setPipeline(p => p.map((x, i) => i === selected ? { ...x, options: o } : x))} /></>}</Box>
      </Box>
    </main>
