...
Bun.serve({
  ...
  routes: {
    ...
    "/assets/main.css": new Response(
      Bun.file(isProduction ? "./dist/assets/main.css" : "./src/style.css"),
      { headers: { "Content-Type": "text/css" } },
    ),
    "/src/style.css": new Response(Bun.file("./src/style.css"), {
      headers: { "Content-Type": "text/css" },
    }),
  },
  async fetch(request) {
...
console.log(
  `Document cropper running at http://localhost:${process.env.PORT || 3000}`,
);
